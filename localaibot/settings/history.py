...
class HistorySettings(MixinMeta):
    @aibot.group(name="history", aliases=["context"])
    @checks.is_owner()
    ...
    async def history_maxtokens(self, ctx: commands.Context, new_value: Optional[int]):
        """
        Set a LLM's custom maximum context limit (for local LLMs or those not listed in `aibot/common/constants.py`.).

        If not set, a safe default or saved limit from `aibot/common/constants.py` is used.
        """
