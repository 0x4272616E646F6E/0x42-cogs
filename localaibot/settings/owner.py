...
from localaibot.config.defaults import DEFAULT_LLM_MODEL
from localaibot.core.openai_utils import setup_openai_client
from localaibot.settings.utilities import get_tokens, truncate_prompt
from localaibot.types.abc import MixinMeta
from localaibot.utils.utilities import (
...
class OwnerSettings(MixinMeta):
    @commands.group(aliases=["ai_botowner"])
    @checks.is_owner()
    async def aibotowner(self, _):
        """For some settings that apply bot-wide."""
        pass

    @aibotowner.command(name="maxpromptlength")
    ...
    @aibotowner.command(name="maxtopiclength")
        ...
        **Arguments:**
        ...
        if restored_count > 0:
            ...
            if restored_count < total_guilds:
                value += f"\nA further {total_guilds - restored_count} servers were set to `{chat_model}` for chat mode."
            embed.add_field(
                name="🔄 Restored",
                value=value,
                inline=False,
            ...
            embed.add_field(
                name="🔄 Reset",
                value=f"All per-server models have been set to use `{chat_model}` for chat mode.",
                inline=False,
            )
    ...
    @aibotowner.command(name="exportconfig")
    async def export_config(self, ctx: commands.Context):
        ...
        await ctx.send(
            file=discord.File(path, filename="aibot_config.json")
        )
        await ctx.tick()

    @aibotowner.command(name="importconfig")
    ...
    @aibotowner.command(name="prompt")
    async def global_prompt(self, ctx: commands.Context, *, prompt: Optional[str]):
        """ Set the global default prompt for aibot.
