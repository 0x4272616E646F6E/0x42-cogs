...
from localaibot.config.defaults import DEFAULT_REMOVE_PATTERNS
from localaibot.types.abc import MixinMeta, aibot
...
class ResponseSettings(MixinMeta):
    ...
    async def remove_weight(self, ctx: commands.Context, word: str):
        ...
        *Arguments*
        ...
        if len(token) == 1:
            ...
            return await ctx.send(":warning: Word is multiple tokens? Please check [p]aibot response weights list")
    ...
    async def set_custom_parameters(self, ctx: commands.Context, *, json_block: str):
        """ Set custom parameters for an endpoint using a JSON code block
            ...
            To reset parameters to default, use `[p]aibot response parameters reset`
            To show current parameters, use `[p]aibot response parameters show`

            Example command:
            `[p]aibot response parameters ```{"frequency_penalty": 2.0, "max_tokens": 200}``` `
        ...
        if json_block not in ['show', 'list']:
            ...
            if data.get("logit_bias") and await self.config(ctx.guild).weights():
                embed = discord.Embed(
                    ...
                    description="Wipe existing logit bias (from [p]aibot response weights)?",
                )
                confirm = await ctx.send(embed=embed)
                start_adding_reactions(confirm, ReactionPredicate.YES_OR_NO_EMOJIS)
                pred = ReactionPredicate.yes_or_no(confirm, ctx.author)
