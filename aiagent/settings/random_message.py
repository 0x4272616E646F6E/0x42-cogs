...
from aiagent.config.defaults import DEFAULT_RANDOM_PROMPTS
from aiagent.settings.utilities import get_tokens
...
class RandomMessageSettings(MixinMeta):
    ...
    async def randommessage(self, _):
        """ Configure the random message event
            **(Must be enabled by bot owner first using `[p]aiagent random toggle`)**
