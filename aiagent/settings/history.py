...
class HistorySettings(MixinMeta):
    ...
    async def history_maxtokens(self, ctx: commands.Context, new_value: Optional[int]):
        """
        Set a custom maximum context limit (in tokens) for the model this server uses.

        If not set, the limit is estimated from the model's name, falling back to a
        conservative default. See `aiagent/config/models.py`.

        Leave the value blank to clear the override.
        """
