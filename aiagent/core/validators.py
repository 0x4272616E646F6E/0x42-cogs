...
async def check_llm_client(cog: MixinMeta, _: commands.Context) -> Tuple[bool, str]:
    """Validate and setup the local LLM client"""
    if not cog.openai_client:
        cog.openai_client = await setup_llm_client(cog.bot, cog.config)
        if not cog.openai_client:
...
async def check_channel_settings(cog: MixinMeta, ctx: commands.Context) -> Tuple[bool, str]:
    ...
    # no-op await so this coroutine uses async features (satisfies linters)
...
async def check_user_status(cog: MixinMeta, ctx: commands.Context) -> Tuple[bool, str]:
    ...
    if whitelisted_members or whitelisted_roles:
        user_roles = {role.id for role in ctx.author.roles} if ctx.author.roles else set()
        if not (
