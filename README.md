...
python -m pip install "Red-DiscordBot>=3.5.1" "openai>=2.0,<3" "pydantic>=2.7,<2.12" "httpx>=0.27,<1"
```

`discord.py` is intentionally not listed: Red pins it exactly (`discord-py==2.7.1`), and
installing a different version alongside it breaks the bot. `pydantic` is capped below
2.12 for the same reason — see
